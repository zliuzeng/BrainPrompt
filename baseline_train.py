...
def make_model_baseline_model(N=1, d_model1=6670, d_model2=28, d_ff=128, h1=2, dropout=0.5, device=0):
    c = copy.deepcopy
    ...
    model = baseline_model.EncoderDecoder(
            baseline_model.Encoder(baseline_model.EncoderLayer(d_model1, d_model2, c(attn1), c(ff), dropout), N),
...
def train_and_test_baseline_model(args, path_data, path_data_mask, path_label, lr, weight_decay):
    ...
    for train_index, test_index in kf.split(X):
        ...
        model = make_model_baseline_model(N=1, d_model1=6670, d_model2=25, d_ff=128, h1=2,dropout=args.dropout, device=device)

        # #根据选择的模型不同，设定不同模型的初始化和选择更新的参数
        model, model_param_group, optimizer = configure_model_optimizer_baseline_model(args, model, device,lr, weight_decay)
        ...
        for epoch in range(0, args.epoch_cf + 1):
            ...
            if epoch == args.best_epoch and args.save_model == 1:
                if args.model == 'baseline_model':
                    save_path = f'./baseline_model/{args.site}/15_site/{kfold_index}.pt'
