...
import source_model2,source_model1,source_model12
import xlwt
...
class LateFusionDistillationModel(nn.Module):
    def __init__(self, student_model, teacher_models,num_sources,weights=None):
        """
    ...
    def forward(self, x, src_mask):
        ...
        for i, teacher_model in enumerate(self.teacher_models):
            pred,_ = teacher_model(x, src_mask)
        ...
        # 获取学生模型的输出
        student_output,_ = self.student_model(x, src_mask)
...
def make_model(N=6, d_model1=72, d_model2=116, d_ff=2048, h1=8, dropout=0.5, device=0, num_prompts=3):
    """
    ...
    attn1 = source_model1.MultiHeadedAttention(h1, d_model1, device)
    ff = source_model1.PositionwiseFeedForward(d_model1, d_ff, dropout)
    position = source_model1.PositionalEncoding(d_model1, dropout)

    # 创建 `num_prompts` 个 Prompt2 对象
    model_sources = [source_model1.EncoderDecoder(
        source_model1.Encoder(source_model1.EncoderLayer(d_model1, d_model2, c(attn1), c(ff), dropout), N),
        c(position), d_model1, d_model2) for _ in range(num_prompts)]

    model_target = source_model1.EncoderDecoder(
        source_model1.Encoder(source_model1.EncoderLayer(d_model1, d_model2, c(attn1), c(ff), dropout), N),
    ...
    return model_sources, model_target
...
for train_index, test_index in kf.split(X):
    ...
    model_sources, model_target = make_model( N=1, d_model1=6670, d_model2=25, d_ff=128, h1=2, dropout=drop_out, device=device, num_prompts=num_sources)

    model_target.load_state_dict(torch.load('./baseline_model/' + str(site1) +'/15_site/'+ str(kfold_index) + '.pt'), strict=False)
    ...
    for i, site in enumerate(source_sites, start=1):
        path=f'./source_model1/{site1}/{site}/{kfold_index}.pt'
        print(path)
        #加载源站点的模型权重
        model_sources[i-1].load_state_dict(torch.load(f'./source_model1/{site1}/{site}/{kfold_index}.pt'), strict=False)
    ...
    fusion_model = LateFusionDistillationModel(model_target, model_sources,num_sources,selected_weights)
    fusion_model.to(device)

    # 冻结所有参数
    for param in fusion_model.student_model.parameters():
        param.requires_grad = False
    # 只更新prompt参数
    for name, param in fusion_model.student_model.named_parameters():
        if "w" in name:
    ...
    for epoch in range(0, epochs + 1):
            ...
            if epoch == best_epoch:
               ...
               #第二种相似度
               save_path='./source_model_2/' + site1 + '/' + str(num_sources) + '/' + str(kfold_index) + '.pt'
