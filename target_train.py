...
def make_model_baseline_model(temperature=8,N=1, d_model1=6670, d_model2=28, d_ff=128, h1=2, dropout=0.5, device=0):
    c = copy.deepcopy
    prompt2 = source_model.Prompt2(d_model1, temperature)
...
def configure_model_optimizer_baseline_model(args, model,prompt2, kfold_index, device, lr, decay):
    """
    配置模型的参数更新和优化器。
    """
    if args.model == 'target_model':
        ...
        prompt2.load_state_dict(torch.load('./source_model_1/'+ str(args.site) + '/'+str(args.num_sources)+'/' + str(kfold_index) + '.pt'), strict=False)
...
def train_and_test_target_train_model(args, path_data, path_data_mask, path_label, lr, weight_decay):
    ...
    for train_index, test_index in kf.split(X):
        ...
        model,prompt2 = make_model_baseline_model(temperature=8,N=1, d_model1=6670, d_model2=25, d_ff=128, h1=2,dropout=args.dropout, device=device)

        # #根据选择的模型不同，设定不同模型的初始化和选择更新的参数
        model, model_param_group, optimizer = configure_model_optimizer_baseline_model(args, model,prompt2,kfold_index,device,lr, weight_decay)
        ...
        for epoch in range(0, args.epoch_cf + 1):
            ...
            for i in range(0, int(X_train.shape[0]) // int(args.batch_size)):
                ...
                outputs = model(train_data_batch, src_mask=X_mask_train_batch,prompt2=prompt2)
                outputs = outputs.cpu()
            ...
            if epoch % 5 == 0:
                ...
                outputs = model(test_data_batch_dev, src_mask=X_mask_test_batch_dev,prompt2=prompt2)
