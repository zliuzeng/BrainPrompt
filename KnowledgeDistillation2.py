...
class LateFusionDistillationModel(nn.Module):
    ...
    def forward(self, x, src_mask):
        ...
        # 获取学生模型的输出
        student_output  = self.student_model(x, src_mask)
...
def make_model(N=6, d_model1=72, d_model2=116, d_ff=2048, h1=8, dropout=0.5, device=0, num_prompts=3):
    ...
    model_sources = [source_model2.EncoderDecoder(
    ...
    model_target = source_model2.EncoderDecoder(
        source_model2.Encoder(source_model2.EncoderLayer(d_model1, d_model2, c(attn1), c(ff), dropout), N),
...
for train_index, test_index in kf.split(X):
    ...
    for i, site in enumerate(source_sites, start=1):
        # 加载源站点的模型权重
        # print(model_sources[i-1])
        model_sources[i-1].load_state_dict(torch.load(f'./source_model2/{site1}/{site}/{kfold_index}.pt'), strict=False)
